...
for(let i = 1; i < size; i++){
  data.push({date:new Date($(".t1"+i).val(), $(".t2"+i).val(), $(".t3"+i).val()), value:$(".data"+i).val()});
}
...
<!-- HTML -->
<input type="hidden" value="${shop_date.size() }" class="shop_size">
<div id="chdiv">
		<form method="post">
...
<c:forEach var="data"  items="${order_date }">
			<tr>
				...
				<td>
				<input type="text" value="${data.tot }원" class="data${data.rownum } dise" readonly style="width: 100%"></td>
			</tr>
</c:forEach>
		</tbody>
	</table>
